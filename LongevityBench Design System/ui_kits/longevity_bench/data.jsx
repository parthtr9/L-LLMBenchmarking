...
async function loadDashboardData() {
  try {
    ...
    const records = d.samples.map((s, i) => {
      const llm = s.cells?.longevity_llm || {};
      return {
        row: i,
        lbId: s.lb_id,
        ...
        gene1: s.metadata?.gene_1 || s.metadata?.gene1 || '—',
        ...
        consistent: llm.pass,
        faithfulness: null,
        sourceDb: s.metadata?.source_db || 'LongeBench',
        sourceId: s.id,
        userMsg: s.id + ' · gold: ' + s.gold,
        trace: llm.trace || '',
      };
    });
